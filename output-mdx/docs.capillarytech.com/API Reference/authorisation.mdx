...
````
   ...
   **Solution**: Make sure that the endpoint is valid and appears under the selected **Resource** in the access group and that the assigned access group for the store has the appropriate permissions to perform the requested operation.
2. **Error**:
   ```json
   HttpStatusCode=503
