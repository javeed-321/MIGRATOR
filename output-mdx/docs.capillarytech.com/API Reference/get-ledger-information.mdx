...
| * customerId | Integer | Unique ID of the customer who triggered the event. When type=USERGROUP2, this may return a negative value — this is expected behaviour. The absolute value of the negative number is the group ID. To query group-level ledger entries, use groupId as the identifierName parameter instead of customerId. |
| * ledgerCreatedDate | String | Indicates the ledger entry created date in YYYY-MM-DD HH:MM:SS.s format. **Note:** This field will be deprecated in the future and is being replaced by `ledgerCreatedDateISO`. |
| * ledgerCreatedDateISO | String | Indicates the ledger entry created date in ISO 8601 format, returned in the server timezone. **EU server example ** 2025-12-16T14:30:45Z → 16 December 2025, 14:30:45 (UTC) **India server example** 2025-12-16T14:30:45+05:30 → 16 December 2025, 14:30:45 (IST) Note: The response timezone always matches the server timezone, regardless of the timezone offset in the request. |
...
| --- value | String | Registered identifier of the customer in the user group based on the type. Character limit for email: 255 characters. |
| --- role | String | Role of the customer in the user group. For example, a company enrolled in a fleet loyalty program can have roles such as owner, associate, driver. |
...
| -- ledgerEntryType | Enum | Type of ledger entry. Supported values: OPENING_BALANCE, CREDIT, DEBIT. Refer to the documentation on points liability for more details. |
| -- points | Float | Number of points transacted. |
| -- pointsCategory | String | Category of points transacted. Supported values: Main (redeemable account), DelayedAccrualPointCategory (promised points), ExternalTriggerBasedPointCategory (points awarded based on external events or triggers.). Refer to the documentation on points for more information. |
...
| -- "name of alternate currency" | Float | Net amount of alternate currencies transacted. The parameter is the unique name of the alternate currency for the organisation. |
...
| -- eventDate | String | Indicates the event-triggered date in YYYY-MM-DD HH:MM:SS.s format. **Note:** This field will be deprecated in the future and is being replaced by `eventDateISO`. |
| --eventDateISO | String | Indicates the event-triggered date and time in ISO 8601 format, returned in the server timezone. **EU server example**: 2025-12-16T14:30:45Z → 16 December 2025, 14:30:45 (UTC). **India server example**: 2025-12-16T14:30:45+05:30 → 16 December 2025, 14:30:45 (IST). **Note:** The response timezone always matches the server timezone, regardless of the timezone offset in the request. |
...
| * conceptId | Integer | Unique ID of the concept associated with the store where the transaction occurred. Returned only when `includeConceptDetails=true`. Returns `-1` if the concept or store cannot be resolved for the till. |
| * conceptName | String | Name of the concept associated with the store where the transaction occurred. Returned only when `includeConceptDetails=true` and the concept has a valid name. Omitted from the response if the concept name is unavailable. |
