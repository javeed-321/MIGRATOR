...
| --- | --- | --- |
| `issuedToIds` | Integer | Pass the customer ID that you want to retrieve the coupon details from. |
...
| `coupon_code_case` | Enum | This parameter helps retrieve the coupon code in the desired letter case. By default, the system converts the coupon code to upper case. The query itself is not case sensitive. **Supported values:** `LOWER`: Converts to lower case. \ `UPPER`: Converts to upper case. \ `AS_IT_IS`: Retrieves the code exactly as created. For instance, if the code is "abAB123," it should be retrieved and displayed as "abAB123," maintaining its original formatting. |
...
| code               | Coupon code.                                                               |
| valid_till        | Expiration date of the coupon (`"DD-MM-YYYY"`).                            |
| issued_on         | Timestamp when the coupon was issued (`"YYYY-MM-DD HH:MM:SS"`).            |
| valid_from        | Start date of the coupon validity (`"YYYY-MM-DD HH:MM:SS"`).               |
| purpose            | Purpose or additional description of the coupon (may be blank).            |
| custom_properties | Custom properties associated with the coupon.                              |
| name               | Name of a custom property.                                                 |
| value              | Value of the custom property.                                              |
| series_id         | Unique identifier of the coupon series.                                    |
| is_absolute       | Indicates if the coupon value is an absolute discount (true/false).        |
| value              | Value of the coupon (e.g., `10`).                                          |
| discount_upto     | Maximum discount limit applicable with the coupon.                         |
...
| item_status       | Status information for the specific coupon processing.                     |
...
| redemption_info   | Details regarding redemption if the coupon was redeemed.                   |
| redeemed           | Indicates whether the coupon was redeemed.                                 |
| id                 | Redemption ID.                                                             |
| redeemed_on       | Timestamp when the coupon was redeemed (`"YYYY-MM-DD HH:MM:SS"`).          |
| redeemed_at       | Store or location where the coupon was redeemed.                           |
| redeemed_by       | Information about the person who redeemed the coupon.                      |
| store              | Store information where the coupon was redeemed.                           |
| transaction        | Transaction details for the coupon redemption.                             |
| issued_store      | Store information where the coupon was originally issued.                  |
