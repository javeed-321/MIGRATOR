...
* Points Report (New) - This report provides a comprehensive analysis of loyalty points issued, redeemed, expired, and returned. It helps in providing valuable insights into the dynamics of loyalty program engagement.\ <Callout icon="📘" theme="info">
  Note:

  Insights+ counts each point transaction separately to track detailed redemption activity, while Member care extracts the group related transactions to show actual customer redemptions. For example, if a customer redeems points for a shirt, pants, and shoes in one shopping trip, Insights+ will count 3 events (one per item), while Member Extract will count 1 event (the shopping trip). Both methods show the same total points spent, but the event counts differ to serve different reporting needs. </Callout>
