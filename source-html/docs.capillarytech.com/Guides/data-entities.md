...
<Image align="center" className="border" width="smart" border={true} src="https://files.readme.io/63975ea-NGP9ApbSolqFPXoYY3ZOwlUneaAG2HI61Q_1.png" />
...
<Image align="center" className="border" width="smart" border={true} src="https://files.readme.io/36f8462-ZWDw6OuJQNhrsbLCLQt7UY4EBgvDDsWNkg.png" />
...
Extended Fields are proposed fields to standardize input values and keys across organizations. These fields help you capture consistent data across different modules. You can define the field names, data types, enum values, and scopes for extended fields, and capture values based on rules (logical or regex expressions).

An extended field is a predefined field with an id, name, entity type, label name, and data type. Examples include GUID, IMEI, and other identifiers. Extended fields are used in customer registration, profile updates, transactions, and transaction line items.

**Note:** For bill or lineitem entities, if you use Integer type for an extended field, any null values in the source data are converted to 0 during ETL processing. This means nulls are not preserved. If you need to keep null values, use String type for the extended field.

When you add an extended field to collect additional information, it starts accumulating data in the system. However, this data is not immediately visible in the user interface for audience creation.
If you want to build an audience using filters based on the information stored in these extended fields, you need to raise a request to the Customer Success Team to create these filters. Once implemented, you can use these filters to create audiences. For more information, see [audience creation using filters](https://docs.capillarytech.com/docs/audience-management#using-audience-filters).
